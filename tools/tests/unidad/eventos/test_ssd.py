"""Trasladado de tools/test_ssd_records.py (F1.5, #46); registros SSD sintéticos y redistribuibles."""
...
class RecordsTests(unittest.TestCase):
    ...
    def test_byte_length_limit_is_not_silently_truncated(self):
        ...
        with self.assertRaises(ValueError):
            S.replace(sample(), {0: b"A" * 248})

    def test_reject_embedded_nul_and_bad_index(self):
        with self.assertRaises(ValueError):
            S.replace(sample(), {0: b"a\0b"})
        with self.assertRaises(ValueError):
            S.replace(sample(), {1: b"text"})
        bad = bytearray(sample())
        struct.pack_into("<I", bad, 44, 1)
        with self.assertRaises(ValueError):
            S.parse(bad)
