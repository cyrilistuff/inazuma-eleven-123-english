"""Trasladado de tools/test_ui_formats.py (F1.5, #46); solo fixtures sintéticos."""
...
from ie123kit.nucleo.graficos.ctpk import decode, encode
from ie123kit.nucleo.graficos.pintado import paint
from ie123kit.nucleo.graficos.qna import regions
...
def fuente_prueba():
    """Devuelve una TTF negrita del sistema (Arial en Windows, DejaVu en Linux) o salta el test."""
    for ruta in FUENTES_CANDIDATAS:
        if Path(ruta).is_file():
            return ruta
    pytest.skip("no hay fuente de prueba disponible: " + ", ".join(FUENTES_CANDIDATAS))
